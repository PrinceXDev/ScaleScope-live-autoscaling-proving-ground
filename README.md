...
infrastructure (see `docs/testing.md`). The web console is built and builds
cleanly. See `docs/deploy.md` for the fastest path to a live, public demo, and
the "what's next" section below for what's genuinely still in progress.
...
- ClickHouse-backed reads (`timeline`, `instances`, `compare`, degraded replay)
  ...
  ClickHouse — see `docs/testing.md` for exactly what has and hasn't been verified.
...
in `docs/testing.md` before they reached a live deploy.
...
`docs/testing.md` for why that tag being correct is itself a thing worth checking).
